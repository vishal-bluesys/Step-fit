...
.controller('TrendsCtrl', function($ionicLoading, $http, $scope, $timeout, $stateParams, ionicMaterialInk , $ionicModal, $ionicPopover, localStorageService) {
    //$scope.$parent.clearFabs();
...
// Steps total  
  $scope.trendstepmonth = function() {
    ...
    $scope.stepsd.stepsAvg = (totalmonthlySteps / 12).toFixed();
   }
  ...
  $scope.trendstepweekly = function() {
    ...
    $scope.stepsd.stepsAvg = (totalweeklySteps / 7).toFixed();
  }
...
// KM in total
  ...
  $scope.trendkmmonth = function() {
    ...
    $('.stepschart3').hide();
     ...
     $scope.stepsd.kmTotal = (totalmonthlySteps / 1250).toFixed(2);
    $scope.stepsd.kmAvg = ((totalmonthlySteps / 1250) / 12).toFixed(2);
   }
  ...
  $scope.trendkmweekly = function() {
    ...
    $('.stepschart3').hide(); 
       ...
       $('.stepweekly').addClass('activetrends');
       $('.kmdaily').removeClass('activetrends');
    $scope.stepsd.kmTotal = (totalweeklySteps / 1250).toFixed(2);
    $scope.stepsd.kmAvg = ((totalweeklySteps / 1250) / 7).toFixed(2);
  }
...
// calories in total
  ...
  $scope.trendcaloriesmonth = function() {
    ...
    $('.stepschart3').hide();   
      ...
      $scope.stepsd.calTotal = (totalmonthlySteps * 0.044).toFixed(2);
    $scope.stepsd.calAvg = ((totalmonthlySteps * 0.044) / 12).toFixed(2);
   }
  ...
  $scope.trendcaloriesweekly = function() {
   ...
   $('.stepschart1').hide();
    ...
    $('.stepschart3').hide();
       ...
       $('.stepweekly').addClass('activetrends');
       $('.caloriesdaily').removeClass('activetrends');
    $scope.stepsd.calTotal = (totalweeklySteps * 0.044).toFixed(2);
    $scope.stepsd.calAvg = ((totalweeklySteps * 0.044) / 7).toFixed(2);
  }
      ...
      weeklyData[weeklyDatatemp[x].activity_date] = weeklyDatatemp[x].step_count;
        ...
        $.each(weeklyData, function(key, value){
          ...
          if(value == 0)
            ...
            weeklyKmData.push(parseFloat((parseInt(value) / 1250).toFixed(2)));
          weeklyCalData.push(parseFloat((parseInt(value) * 0.044).toFixed(2)));
        });
$scope.stepsd.weeklyTotal = totalweeklySteps;
        $scope.stepsd.weeklyAvg = (totalweeklySteps / 7).toFixed();
    $scope.labels2 = weeklyKeyArray;
        $scope.data2 = [weeklyValueArray];
        //$scope.data2 = [weeklyValueArray];
        $scope.data5 = [weeklyKmData];
        $scope.data8 = [weeklyCalData];
    ...
    for (var x=0;x<monthlyDatatemp.length;x++)
    {
      monthlyData[monthlyArray[parseInt(monthlyDatatemp[x].activity_month)-1]] = monthlyDatatemp[x].step_count;
          ...
          $.each(monthlyData, function(key, value){
            ...
            if(value == 0)
              monthlyKmData.push(0);
            else
              monthlyKmData.push(parseFloat((parseInt(value) / 1250).toFixed(2)));
            monthlyCalData.push(parseFloat((parseInt(value) * 0.044).toFixed(2)));
          });
          $scope.stepsd.monthlyTotal = totalmonthlySteps;
          $scope.stepsd.monthlyAvg = (totalmonthlySteps / 30).toFixed();
        
          $scope.labels1 = monthlyKeyArray;
          $scope.data1 = [monthlyValueArray];
          $scope.data4 = [monthlyKmData];
          $scope.data7 = [monthlyCalData];
...
.controller('NotificationlistCtrl', function($scope, $stateParams, $timeout, ionicMaterialInk, ionicMaterialMotion) {
    // Set Header
    $scope.$parent.showHeader();
   // $scope.$parent.clearFabs();
   // $scope.$parent.setHeaderFab('left');
...
.controller('Newdashboard1Ctrl', function($http,$ionicLoading, localStorageService,$location, $interval, $scope, $timeout, $stateParams, ionicMaterialInk , $ionicModal, $ionicPopover, myservice) {
